    ...
    for i in range(1,len(triangle)): 
        ...
        for j in range(length):
            ...
            if(j != 0 and j != length-1): # 중간에 위치한 값 (화살표 2개)
								# 오는 것 중 최대 값에다 현재 값 더한 것 넣기
                curmax = dp[i-1][j-1] if dp[i-1][j-1]> dp[i-1][j] else dp[i-1][j]
                dp[i].append(curmax + now)
            else: 
								# 제일 끝에 위치한 값이면 이전 제일 끝에 위치한 값 + 현재 값 (화살표 1개)
                if(j == 0): dp[i].append(dp[i-1][j]+now)
